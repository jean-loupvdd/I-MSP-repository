...
class Monocromator:
    ...
    def resource_root():
        # map waar je data staat, zowel in .py als in .exe
    ...
    def set_wavelength(self, wavelength, calib_file=r"C:\Users\jeans\Desktop\Studie\Stage\MSP\wavelength_calib_Grating_300_500.txt"):
        ...
        # Voeg deze regel toe net boven de interpolatie
        wavelength_corrected = wavelength + 5.372
        target_pos = int(spline(wavelength_corrected))
        ...
        # Controleer of het filter daadwerkelijk moet worden veranderd
        if not hasattr(self, "current_filter_slot"):
            # eerste keer dat de functie draait
            self.current_filter_slot = None

        if slot != self.current_filter_slot:
            # Verander filter alleen als dat nodig is
            if not self.filterPos("filter", slot_number=slot):
                return False

           
      
            # Update de huidige filterpositie
            self.current_filter_slot = slot
